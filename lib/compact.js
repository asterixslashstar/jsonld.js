...
api.compact = ({
  ...
  //   });
  // }
  const typeScopedContext = activeCtx;
  ...
  if(_isObject(element)) {
    ...
    // use any scoped context on activeProperty
    console.log('activeproperty', activeProperty, activeCtx);
    const ctx = _getContextValue(activeCtx, activeProperty, '@context');
    if(!_isUndefined(ctx)) {
      // Note: spec's `from term` var is named `isPropertyTermScopedContext`
      activeCtx = _processContext({
    ...
    for(const type of types) {
      const compactedType = api.compactIri(
        {activeCtx: typeScopedContext, iri: type, relativeTo: {vocab: true}});

      // Use any type-scoped context defined on this value
      const ctx = _getContextValue(
        typeScopedContext, compactedType, '@context');
      if(!_isUndefined(ctx)) {
        activeCtx = _processContext({
          activeCtx,
          localCtx: ctx,
    ...
    for(const expandedProperty of keys) {
      ...
      if(expandedProperty === '@id' || expandedProperty === '@type') {
        ...
        const valueContext = isType ? typeScopedContext : activeCtx;
