...
api.expand = ({
  ...
  const propertyScopedCtx = activeProperty ? _getContextValue(
    activeCtx, activeProperty, '@context') : undefined;
 ...
 * @param options the expansion options.
 * @param insideList true if the element is a list, false if not.
 * @param typeScopedContext any type-scoped context for this object.
 * @param expansionMap(info) a function that can be used to custom map
 *          unmappable values (or to throw an error when they are detected);
 *          if this function returns `undefined` then the default behavior
 *          will be used.
  ...
  for(const key of keys) {
    ...
    if(expandedProperty === '@type') {
      ...
      _addValue(
        ...
        _asArray(value).map(v =>
          _isString(v) ?
            _expandIri(typeScopedContext || activeCtx, v,
    ...
    if(container.includes('@language') && _isObject(value)) {
      ...
      expandedValue = _expandLanguageMap(activeCtx, value, options);
      ...
      expandedValue = _expandIndexMap({
        ...
        activeCtx: activeCtx.previousContext || activeCtx,
        options,
        activeProperty: key,
        value,
        expansionMap,
    ...
    if(activeCtx.mappings.has(key) && activeCtx.mappings.get(key).reverse) {
      const reverseMap =
